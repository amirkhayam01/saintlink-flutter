...
void main() {
  ...
  ProviderContainer containerWith(FakePaymentService payments) => ProviderContainer.test(overrides: [
  ...
  test('a completed sheet marks the booking paid and refetches it', () async {
    ...
    final subscription = container.listen(tripDetailProvider('SL-TEST'), (_, _) {});
    ...
    expect((await container.read(tripDetailProvider('SL-TEST').future)).canPay, isFalse);
    subscription.close();
  });
