...
// Renders every screen to test/screenshots/out/ with real fonts.
// Skipped normally; run: SCREENSHOTS=1 flutter test test/screenshots --update-goldens
    ...
    figtree.addFont(
      Future.value(
        ByteData.sublistView(
          File('assets/fonts/Figtree-$w.ttf').readAsBytesSync(),
        ),
      ),
    );
  }
...
void main() {
  ...
  Booking sample(
    ...
    int days = 4,
    bool paid = true,
  }) => bookingWith(reference: ref, canPay: !paid).copyWith(
    ...
    fareItems: const [
      FareItem(label: 'Outward journey', amount: 155),
      FareItem(label: 'Return journey', amount: 155),
    ],
    legs: [
      BookingLeg(
        id: 1,
        ...
        requestedPickupAt: DateTime.now().add(
          Duration(days: days, hours: 2, minutes: 40),
        ),
        vehicle: 'Executive Saloon',
        flight: const BookingFlight(
          number: 'BA123',
          terminal: 'T5',
          status: 'On time',
        ),
        stops: const [
          BookingStop(type: 'pickup', address: 'Southampton Central Station'),
          BookingStop(type: 'dropoff', address: 'Heathrow Airport Terminal 5'),
        ],
      ),
      BookingLeg(
        id: 2,
        ...
        stops: const [
          BookingStop(type: 'pickup', address: 'Heathrow Airport Terminal 5'),
          BookingStop(type: 'dropoff', address: 'Southampton Central Station'),
        ],
      ),
    ],
  );

  ProviderContainer container({RecentPlaces Function()? recents}) =>
      ProviderContainer(
        overrides: [
  ...
  Future<void> shot(
    ...
    Widget home, {
    ThemeData? theme,
    Future<void> Function(ProviderContainer)? prime,
    bool tall = false,
    RecentPlaces Function()? recents,
  ...
  testWidgets('journey date time picker', (tester) async {
    ...
    await tester.pumpAndSettle();
    showJourneyDateTimeSheet(
      tester.element(find.byType(JourneyScreen)),
      title: 'Pickup date & time',
      minimum: DateTime.now(),
      initial: DateTime.now().add(const Duration(days: 1, hours: 1)),
    );
    await tester.pumpAndSettle();
    await expectLater(
      find.byType(MaterialApp),
      matchesGoldenFile('out/journey_date_time_picker.png'),
    );
  });
  ...
  testWidgets(
    'prices',
    (t) => shot(t, 'prices', const PricesScreen(), tall: true),
  );
  testWidgets(
    'journey dark',
    (t) => shot(
  ...
  testWidgets(
    'prices dark',
    (t) => shot(t, 'prices_dark', const PricesScreen(), theme: AppTheme.dark()),
  );
  testWidgets(
    'confirmation',
    (t) => shot(
    ...
    (t) => shot(
      ...
      prime: (c) async {
        bookings.pages = [
          [
            sample('SL-8K2M'),
            sample('SL-7QPA', days: 12, paid: false),
            sample('SL-2BXA', days: -20),
          ],
        ];
      },
    ),
  ...
  testWidgets(
    'trips dark',
    (t) => shot(t, 'trips_dark', const TripsScreen(), theme: AppTheme.dark()),
  );
  testWidgets(
    'trip detail',
    (t) => shot(
      t,
      'trip_detail',
      const TripDetailScreen(reference: 'SL-8K2M'),
      prime: (c) async {
...
/// Recents without SharedPreferences, whose channel never answers in a widget test.
